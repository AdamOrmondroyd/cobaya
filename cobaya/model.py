...
class LogPosterior:
    ...
    def _logpost_is_consistent(self):
        ...
        if self.finite:
            return np.isclose(
                np.nan_to_num(self.logpost), np.nan_to_num(self.logprior + self.loglike)
            )
        else:
...
class Model(HasLogger):
    ...
    **NB:** do not initialize this class directly; use :func:`~model.get_model` instead,
    ...
    def _to_sampled_array(
        self, params_values: Union[Dict[str, float], Sequence[float]]
    ) -> np.ndarray:
        """
        ...
        if return_derived or return_output_params:
            if not compute_success:
                ...
                if return_output_params:
                    return_params = (
                        outpar_dict if as_dict else list(outpar_dict.values())
                    )
        ...
        if _no_check:
            ...
            if self.is_debug():
                self.log.debug(
                    ...
                    dict(
                        zip(self.parameterization.sampled_params(), params_values_array)
                    ),
                )
    ...
    def _set_component_order(self, components, dependencies):
        ...
        comps = [
            c for c in components if not isinstance(c, AbsorbUnusedParamsLikelihood)
        ]
    ...
    def _set_dependencies_and_providers(
        ...
        def _tidy_requirements(_require, _component=None):
            ...
            requirements_in_input_params = set(
                req.name for req in _require
            ).intersection(self.input_params)
        ...
        self._must_provide: Dict[Theory, List[Requirement]] = {
                ...
                for requirement in requires:
                    ...
                    if not suppliers:
                        ...
                        requirements[manual_theory] = [
                            req
                            for req in requirements[manual_theory]
                ...
                if (
                    ...
                    for comp in components:
                        if (
                            ...
                            and component in self._dependencies.get(comp, [])
                        ):
    ...
    def _assign_params(
        ...
        for assign, option, prefix, derived_param in (
            ...
            for component in assign_components:
                if derived_param:
                    required_params = set(
                        str_to_list(getattr(component, "provides", []))
                    )
                else:
                    required_params = set(
                        p
        ...
        for p in output_assign:
            if p.startswith(get_chi2_name("")):
                ...
                if index is None:
                    raise LoggedError(
                        self.log,
                        "Your derived parameters depend on an unknown "
                        "likelihood: '%s'",
                        like,
                    )
                if p in chi2_names:
        ...
        for assign, option in (
            (input_assign, "input_params"),
    ...
    def get_param_blocking_for_sampler(self, split_fast_slow=False, oversample_power=0):
        ...
        # b) 2-block slow-fast separation
            ...
            self.log.debug(
                "Doing slow/fast split. The oversampling factors for the fast "
            ...
            for theory in self.theory.values():
                if hasattr(theory, "get_auto_covmat"):
                    return theory.get_auto_covmat(
                        params_info,
                        self.info()["likelihood"],
                        random_state=random_state,
                    )
        except Exception as e:
            self.log.warning(
                "Something went wrong when looking for a covmat: %r", str(e)
            )
            return None
...
def get_model(
    ...
    """
    info = load_info_overrides(
        info_or_yaml_or_file, debug, stop_at_error, packages_path, override
    )
...
def load_info_overrides(
    info_or_yaml_or_file, debug, stop_at_error, packages_path, override=None
) -> InputDict:
