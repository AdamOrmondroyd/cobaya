...
class DummyModel:
    def __init__(self, info_params, info_likelihood, info_prior=None):
        self.parameterization = Parameterization(
            info_params, ignore_unused_sampled=True
        )
...
def post(
    ...
    if info.get("modules"):
        raise LoggedError(
            log,
            "The input field 'modules' has been deprecated." "Please use instead %r",
            packages_path_input,
        )
    ...
    if not info.get("output") and info_post.get("output") and not info.get("params"):
        raise LoggedError(
            log,
            "The input dictionary must have be a full option "
    ...
    chi2_names_add = [
        get_chi2_name(name) for name in add["likelihood"] if name != "one"
    ]
    ...
    model_add = Model(
        ...
        packages_path=(
            info_post.get(packages_path_input) or info.get(packages_path_input)
        ),
        allow_renames=False,
        post=True,
