...
def install(*infos, **kwargs):
    ...
    for kind, components in used_components.items():
        for component in components:
            ...
            print(
                create_banner(
                    kind + ":" + component, symbol=_banner_symbol, length=_banner_length
                ),
                end="",
            )
            print()
            ...
            if isinstance(info, str) or "external" in info:
                log.warning(
                    "Component '%s' is a custom function. " "Nothing to do.", component
                )
                continue
            try:
            ...
            if is_installed is None:
                log.info(
                    "%s.%s is a fully built-in component: nothing to do.",
                    kind,
            ...
            # test installation
            with NoLogging(None if debug else logging.ERROR):
                successfully_installed = is_installed(
                    path=install_path, check=False, **kwargs_install
                )
...
def download_file(url, path, no_progress_bars=False, decompress=False, logger=None):
    ...
    with tempfile.TemporaryDirectory() as tmp_path:
        try:
            ...
            logger.error("Error downloading %s' to folder '%s': %s", url, tmp_path, e)
            return False
        ...
        try:
            ...
            logger.error(
                "Error decompressing downloaded file! Corrupt file? [%s]", excpt
            )
            return False
...
def install_script(args=None):
    ...
    if getattr(arguments, modules) is not None:
        raise LoggedError(
            logger,
            "-m/--modules has been deprecated in favor of " "-%s/--%s",
            packages_path_arg[0],
            packages_path_arg_posix,
        )
    ...
    if arguments.just_check is True:
        raise LoggedError(
            logger, "--just-check has been deprecated in favor of --%s", "test"
        )
    # END OF DEPRECATION BLOCK
    # Launch installer
    install(
