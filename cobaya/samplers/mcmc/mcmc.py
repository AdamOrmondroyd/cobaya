...
class MCMC(CovmatSampler):
    ...
    def initialize(self):
        ...
        if getattr(self, "oversample", None) is not None:
            raise LoggedError(
                self.log,
                "`oversample` has been deprecated. "
        ...
        if getattr(self, "check_every", None) is not None:
            raise LoggedError(
                self.log,
                "`check_every` has been deprecated. "
        ...
        if self.output.is_resuming() and len(self.collection):
            ...
            if self.measure_speeds and self.blocking:
                ...
                self.model.measure_and_set_speeds(
                    n=n, discard=0, random_state=self._rng
                )
    ...
    def i_last_slow_block(self):
        if self.drag:
            return (
                next(i for i, o in enumerate(self.oversampling_factors) if o != 1) - 1
            )
    ...
    def set_proposer_blocking(self):
        ...
        if self.drag:
            ...
            if getattr(self, "drag_limits", None) is not None:
                raise LoggedError(
                    self.log,
                    "`drag_limits` has been deprecated. "
            ...
            if self.oversample_thin:
                self.current_point.output_thin = int(
                    np.round(
                        sum(
                            len(b) * o
                            for b, o in zip(self.blocks, self.oversampling_factors)
                        )
                        / self.model.prior.d()
                    )
                )
        ...
        blocks_indices = [
            [sampled_params_list.index(p) for p in b] for b in self.blocks
        ]
    ...
    def get_new_sample_metropolis(self):
        ...
        accept = self.metropolis_accept(
            trial_results.logpost, self.current_point.logpost
        )
    ...
    def check_convergence_and_learn_proposal(self):
        ...
        if is_main_process():
            ...
            try:
                ...
                try:
                    ...
                    converged_means = (
                        max(Rminus1, self.Rminus1_last) < self.Rminus1_stop
                    )
        else:
            mean_of_covs = None
            success_means = None
        ...
        if converged_means:
            ...
            if is_main_process():
                if success_bounds:
                    Rminus1_cl = np.std(bounds, axis=0).T / np.sqrt(
                        np.diag(mean_of_covs)
                    )
    ...
    def do_output(self, date_time):
        ...
        if (
            self.burn_in_left and self.burn_in
        ):  # NB: burn_in_left = 1 even if no burn_in
    ...
    def _get_desc(cls, info=None):
        drag_string = (
            r" using the fast-dragging procedure described in \cite{Neal:2005}"
        )
        if info is None:
