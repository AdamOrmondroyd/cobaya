...
class BatchArgs:
    def __init__(
        ...
        converge=False,
        plots=False,
        batchPathOptional=False,
    ):
        self.parser = argparse.ArgumentParser(prog=prog, description=desc)
        if not noBatchPath:
            if batchPathOptional:
                self.parser.add_argument(
                    "batchPath", nargs="?", help="directory containing the grid"
                )
            else:
                self.parser.add_argument(
                    "batchPath", help="directory containing the grid"
                )
        if converge:
            self.parser.add_argument(
                "--converge", type=float, default=0, help="minimum R-1 convergence"
            )
    ...
    def parseForBatch(self, vals=None):
        ...
        if self.doplots:
            ...
            self.parser.add_argument(
                "--paramList",
                default=None,
                help=".paramnames file listing specific parameters "
                "to include (only)",
            )
            self.parser.add_argument(
                "--size_inch",
            ...
            self.parser.add_argument(
                "--nx", default=None, help="number of plots per row"
            )
            self.parser.add_argument(
                "--outputs",
                nargs="+",
    ...
    def wantImportance(self, jobItem):
        return (
            self.args.importancetag is None
            or len(self.args.importancetag) == 0
            or jobItem.importanceTag in self.args.importancetag
        ) and (
            self.args.importance is None
            or len(self.args.importance) == 0
            or len(
                [
                    True
                    for x in self.args.importance
                    if x in jobItem.data_set.importanceNames
                ]
            )
        )
    ...
    def jobItemWanted(self, jobItem):
        return (
            ...
            and (self.args.importance is None)
            or jobItem.isImportanceJob
            and self.wantImportance(jobItem)
        )
    ...
    def nameMatches(self, jobItem):
        ...
        for pat in self.args.name:
            if fnmatch.fnmatch(jobItem.name, pat):
                return True
        return False
    ...
    def filteredBatchItems(self, wantSubItems=True, chainExist=False):
        for jobItem in self.batch.items(
            wantImportance=not self.args.noimportance, wantSubItems=wantSubItems
        ):
            if (not chainExist or jobItem.chainExists()) and (
                self.jobItemWanted(jobItem)
                and self.nameMatches(jobItem)
                and self.paramsMatch(jobItem)
                and self.dataMatches(jobItem)
    ...
    def filterForDataCompare(self, batch, datatags, getDistExists=False):
        items = []
        for tag, data in zip(
            [self.batch.normalizeDataTag(data) for data in datatags], datatags
        ):
            items += [
                jobItem
                for jobItem in batch
