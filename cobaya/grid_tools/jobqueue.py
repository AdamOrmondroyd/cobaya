...
def addArguments(parser, combinedJobs=False):
    ...
    parser.add_argument(
        "--qsub", help="option to change qsub command to something else"
    )
    parser.add_argument(
        "--dryrun",
        action="store_true",
...
class JobSettings:
    ...
    def __init__(self, jobName, msg=False, **kwargs):
        ...
        self.walltime = get_defaulted(
            "walltime", "24:00:00", template=template, **kwargs
        )
...
def queue_job_details(batchPath=None, running=True, queued=True, warnNotBatch=True):
    ...
    if spawn.find_executable("showq") is not None:
        ...
        running_txt = " r "
    res_str = (
        subprocess.check_output("%s -u $USER" % qstat, shell=True).decode().strip()
    )
    res = res_str.split("\n")
    names = []
    job_names = []
