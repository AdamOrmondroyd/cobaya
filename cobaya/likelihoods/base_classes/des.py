...
class DES(DataSetLikelihood):
    ...
    def init_params(self, ini):
        ...
        for b in range(self.nzbins):
            self.zbin_sp += [
                InterpolatedUnivariateSpline(self.zmid, nz_source[:, b + 3])
            ]
        ...
        for b in range(self.nwbins):
            self.zbin_w_sp += [
                InterpolatedUnivariateSpline(self.zmid, nz_lens[:, b + 3])
            ]
        ...
        if any(t in self.used_types for t in ["gammat", "xim", "xip"]):
            ...
            if PKWeyl is not None:
                if "gammat" in self.used_types:
                    raise LoggedError(
                        self.log,
                        "DES currently only supports Weyl "
        ...
        if self.use_hankel:  # pragma: no cover
            ...
            if "xip" in self.used_types or "xim" in self.used_types:
                for (f1, f2) in self.bin_pairs[self.data_types.index("xip")]:
                    ...
                    corrs_th_p[f1, f2] = (
                        self.hankel0.transform(
                            cl, self.theta_bins_radians, ret_err=False
                        )
                        * fac
                    )
                    corrs_th_m[f1, f2] = (
                        self.hankel4.transform(
                            cl, self.theta_bins_radians, ret_err=False
                        )
                        * fac
                    )
    ...
    def logp(self, **params_values):
        PKdelta = self.provider.get_Pk_interpolator(
            ("delta_tot", "delta_tot"), extrap_kmax=500 * self.acc
        )
        if self.use_Weyl:
            PKWeyl = self.provider.get_Pk_interpolator(
                ("Weyl", "Weyl"), extrap_kmax=500 * self.acc
            )
        else:
            PKWeyl = None
    ...
    def plot_lensing(
        ...
        for f1, f2 in self.bin_pairs[0]:
            ...
            if corrs_p is not None and not diff:
                ...
                ax.semilogx(
                    self.theta_bins, facm * corrs_m[f1, f2], color="C1", ls="--"
                )
