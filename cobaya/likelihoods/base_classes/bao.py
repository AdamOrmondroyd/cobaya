...
class BAO(InstallableLikelihood):
    ...
    def initialize(self):
        ...
        if self.measurements_file:
            try:
                ...
                raise LoggedError(
                    self.log,
                    "Couldn't find measurements file '%s' in folder '%s'. "
        ...
        if self.prob_dist:
            ...
            try:
                ...
                raise LoggedError(
                    self.log,
                    "Couldn't find grid file '%s' in folder '%s'. "
            ...
            if self.grid_data.shape[1] == 3:
                ...
                if not self.observable_3:
                    raise LoggedError(
                        self.log,
                        "If using 3D grid data, 'observable_3'"
            ...
            try:
                ...
                raise LoggedError(
                    self.log,
                    "Couldn't find (inv)cov file '%s' in folder '%s'. "
    ...
    def get_requirements(self):
        ...
        if len(obs_used_not_implemented):
            raise LoggedError(
                self.log,
                "This likelihood refers to observables '%s' that have not been"
