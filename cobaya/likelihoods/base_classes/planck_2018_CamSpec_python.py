...
class Planck2018CamSpecPython(DataSetLikelihood):
    ...
    def init_params(self, ini, silent=False):
        ...
        with open(covfile, "rb") as cov_f:
            cov = np.fromfile(
                cov_f, dtype=[np.float32, np.float64]["64.bin" in covfile]
            )
        ...
        if np.any(self.cl_used[:4]):
            ...
            self.sz_143 = self.read_normalized(
                ini.relativeFileName("sz143file"), pivot
            )[: max_l + 1]
    ...
    def coadded_TT(
        ...
        for i, (cal, LS, n) in enumerate(
            zip(cals[:4], ell_offsets, self.used_sizes[:4])
        ):
