...
def get_covmat_database(packages_path, cached=True) -> List[dict]:
    ...
    installed_folders = [
        ...
        if os.path.exists(folder.format(**{packages_path_input: packages_path}))
    ]
    ...
    for folder in installed_folders:
        ...
        for filename in os.listdir(folder_full):
            try:
                with open(
                    os.path.join(folder_full, filename), encoding="utf-8"
                ) as covmat:
                ...
                continue
            covmat_database.append(
                {"folder": folder, "name": filename, "params": params}
            )
    if cached:
...
def get_best_covmat_ext(
    packages_path, params_info, likelihoods_info, random_state, cached=True
) -> Optional[dict]:
    """
    ...
    covmats_database = _loaded_covmats_database or get_covmat_database(
        packages_path, cached=cached
    )
    ...
    delimiters = r"[_\.]"
    likes_regexps = [
        re.compile(delimiters + re.escape(_like) + delimiters)
        for _like in likes_renames
    ]
    # Match number of params
    score_params = lambda covmat: len(
        set(covmat["params"]).intersection(params_renames)
    )
    ...
    score_likes = lambda covmat: len(
        [0 for r in likes_regexps if r.search(covmat["name"])]
    )
    ...
    score_simpler_name = lambda covmat: -len(
        covmat["name"].replace("_", " ").replace("-", " ").split()
    )
    ...
    if len(best_p_l_sp_sn) > 1:
        log.warning(
            "WARNING: >1 possible best covmats: %r", [b["name"] for b in best_p_l_sp_sn]
        )
