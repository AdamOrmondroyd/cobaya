...
def check_index(i, imax):
    if (i > 0 and i >= imax) or (i < 0 and -i > imax):
        raise IndexError(
            "Trying to access a sample index larger than "
...
class SampleCollection(BaseCollection):
    ...
    def _cache_add_row(
        ...
        """
        self._cache[pos, self._icol[OutPar.weight]] = (
    ...
    def _cache_dump(self):
        ...
        self._data.iloc[
            len(self._data) - (self._cache_last + 1) : len(self._data)
    ...
    def append(self, collection):
        ...
        """
        self._data = pd.concat(
            [self.data[: len(self)], collection.data], ignore_index=True
        )
    ...
    def __getitem__(self, *args):
        ...
        if len(args) > 1:
            raise ValueError(
                "Use just one index/column, or use .loc[row, column]. "
    ...
    def mean(self, first=None, last=None, derived=False, pweight=False):
        ...
        """
        if pweight:
            logps = -self[OutPar.minuslogpost][first:last].to_numpy(
    ...
    def cov(self, first=None, last=None, derived=False, pweight=False):
        ...
        if pweight:
            ...
            weights = self[OutPar.weight][first:last].to_numpy(dtype=np.float64)
            kwarg = (
                "fweights" if np.allclose(np.round(weights), weights) else "aweights"
            )
    ...
    def thin_samples(self, thin, inplace=False) -> "SampleCollection":
        ...
        if thin != int(thin) or thin < 1:
            raise LoggedError(
                self.log, "Thin factor must be an positive integer, got %s", thin
            )
    ...
    def sampled_to_getdist_mcsamples(self, first=None, last=None):
        ...
        with NoLogging():
            mcsamples = MCSamples(
                ...
                weights=self.data[: len(self)][OutPar.weight].to_numpy(
                    dtype=np.float64
                )[first:last],
    ...
    def _dump_slice__txt(self, n_min=None, n_max=None):
        ...
        if not hasattr(self, "_numpy_fmts"):
            ...
            self._numpy_fmts = [
                "%{}.{}".format(width_col(col), n_float) + "g"
                for col in self.data.columns
            ]
            self._header_formatter = [
                eval(
                    "lambda s, w=width_col(col): "
                    '("{:>" + "{}".format(w) + "s}").format(s)',
