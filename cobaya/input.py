...
def load_input_file(
    ...
    no_mpi: bool = False,
    help_commands: Optional[str] = None,
) -> InputDict:
    if no_mpi:
    ...
    if os.path.basename(stem) in ("input", "updated"):
        raise ValueError(
            "'input' and 'updated' are reserved file names. "
    ...
    if suffix.lower() in Extension.yamls + (Extension.dill,):
        ...
        try:
            ...
            if help_commands:
                err_msg += (
                    " Maybe you mistyped one of the following commands: "
                    + help_commands
                )
...
def load_input(input_file: str) -> InputDict:
    ...
    if extension.lower() in Extension.yamls:
        ...
        raise LoggedError(
            log, "Extension of input file '%s' not recognized.", input_file
        )
    ...
    # warn if no output, since we are in shell-invocation mode.
        ...
        log.warning("WARNING: Output explicitly suppressed with '%s: null'", "output")
...
def get_info_path(folder, prefix, infix=None, kind="updated", ext=Extension.yamls[0]):
    ...
    try:
        suffix = {"input": FileSuffix.input, "updated": FileSuffix.updated}[
            kind.lower()
        ]
    except KeyError:
...
def get_used_components(*infos, return_infos=False):
    ...
    for info in infos:
        for kind in kinds:
            try:
                comps[kind] += [
                    a for a in (info.get(kind) or []) if a not in comps[kind]
                ]
            except TypeError:
                raise LoggedError(
                    log,
            ...
            for item in updated_info[name].values():
                ...
                if renames:
                    if not isinstance(renames, Mapping):
                        raise LoggedError(
                            log,
                            "'renames' should be a dictionary of "
                            "name mappings "
...
def merge_params_info(params_infos, default_derived=True):
    ...
    """
    current_info = {
        p: expand_info_param(v, default_derived)
...
def is_equal_info(
    info_old, info_new, strict=True, print_not_log=False, ignore_blocks=()
):
    """
    ...
    if (
        set(info for info in info_old if info_old[info] is not None) - ignore
    ...
    for block_name in info_old:
        ...
        for k in block1:
            ...
            if block1[k] != block2[k]:
                ...
                to_pop = [
                    j for j in block1[k] if (block1[k].get(j) == block2[k].get(j))
                ]
                [(block1[k].pop(j, None), block2[k].pop(j, None)) for j in to_pop]
                myprint(
                    myname
                    + ": different content of [%s:%s]" % (block_name, k)
...
class HasDefaults:
    ...
    def get_bibtex(cls) -> Optional[str]:
        ...
        if filename:
            bib = pkg_resources.resource_string(cls.__module__, filename).decode(
                "utf-8"
            )
        else:
