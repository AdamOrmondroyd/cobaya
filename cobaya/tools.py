...
def check_component_path(component, path):
    if not os.path.realpath(os.path.abspath(component.__file__)).startswith(
        ...
        raise LoggedError(
            log,
            "Component %s successfully loaded, but not from requested path: %s.",
            component.__name__,
            path,
        )
...
def check_component_version(component: Any, min_version):
    if not hasattr(component, "__version__") or version.parse(
        ...
        raise VersionCheckError(
            ...
            % (
                ...
                getattr(component, "__version__", "(non-given)"),
                min_version,
            )
        )
...
def load_module(
    name, package=None, path=None, min_version=None, check_path=False
) -> ModuleType:
...
def get_class(
    ...
    def return_class(_module_name, package=None):
        ...
        if not cls:
            _module = load_module(
                _module_name + "." + _class_name, package=package, path=component_path
            )
    ...
    if None_if_not_found:
        return None
    if exc_info[0] is ModuleNotFoundError and str(exc_info[1]).rstrip("'").endswith(
        name
    ):
        if allow_internal:
            suggestions = fuzzy_match(
                name, get_available_internal_class_names(kind), n=3
            )
            if suggestions:
                raise LoggedError(
                    log,
                    "%s '%s' not found. Maybe you meant one of the following "
        ...
        log.error(
            "There was a problem when importing %s '%s':", kind or "external", name
        )
        raise exc_info[1]


def get_resolved_class(
    component_or_class,
    kind=None,
    component_path=None,
    class_name=None,
    ...
    if isinstance(component_or_class, str):
        component_or_class = get_class(
            class_name or component_or_class,
            kind,
        ...
        if hidden or not name.startswith("_"):
            ...
            if hidden or not getattr(m, "_is_abstract", False):
                ...
                if ispkg:
                    result.update(
                        import_all_classes(
                            os.path.dirname(m.__file__), m.__name__, subclass_of, hidden
                        )
                    )
    return result
    ...
    for cls in classes_in_module(m, subclass_of=CobayaComponent):
        if cls.__name__.lower() in valid_names:
            if result is not None:
                raise ValueError(
                    "More than one class with same lowercase name %s", name
                )
            result = cls
    return result
...
def get_available_internal_classes(kind, hidden=False):
    ...
    return import_all_classes(
        path, "cobaya.%s" % subfolders[kind], CobayaComponent, hidden
    )
...
def get_scipy_1d_pdf(info):
    ...
    if not info2:
        raise LoggedError(
            log, "No specific prior info given for " "sampled parameter '%s'." % param
        )
    # If list of 2 numbers, it's a uniform prior
    elif (
        isinstance(info2, Sequence)
        and len(info2) == 2
        and all(isinstance(n, numbers.Real) for n in info2)
    ):
        info2 = {"min": info2[0], "max": info2[1]}
    elif not isinstance(info2, Mapping):
        raise LoggedError(
            log,
            "Prior format not recognized for %s: %r "
    ...
    try:
        ...
        raise LoggedError(
            log,
            "Error creating the prior for parameter '%s': "
            "The distribution '%s' is unknown to 'scipy.stats'. "
            "Check the list of allowed possibilities in the docs.",
            param,
            dist,
        )
    ...
    if "min" in info2 or "max" in info2:
        ...
        for limit in minmaxvalues:
            ...
            try:
                ...
                raise LoggedError(
                    log,
                    "Invalid value '%s: %r' in param '%s' (it must be a number)",
                    limit,
                    value,
                    param,
                )
        if minmaxvalues["max"] < minmaxvalues["min"]:
            raise LoggedError(
                log,
                "Minimum larger than maximum: '%s, %s' for param '%s'",
                minmaxvalues["min"],
                minmaxvalues["max"],
                param,
            )
    ...
    if not np.all(
        np.isfinite([info2.get(x, 0) for x in ["loc", "scale", "min", "max"]])
    ):
...
def resolve_packages_path(infos=None):
    ...
    for info in infos:
        if info.get("modules"):
            raise LoggedError(
                log,
                "The input field 'modules' has been deprecated."
                "Please use instead %r",
                packages_path_input,
            )
    ...
    if path_old_env and not path_env:
        raise LoggedError(
            log,
            "The env var %r has been deprecated in favor of %r",
            old_env,
            packages_path_env,
        )
    # END OF DEPRECATION BLOCK
