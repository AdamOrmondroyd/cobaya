...
def extract_docs_requirements():
    ...
    path_docs_requirements = path.join(
        path.abspath(path.dirname(__file__)), "docs/requirements.txt"
    )
    with open(path_docs_requirements, "r") as f:
        lines = f.readlines()
    i_first = (
        next(i for i, line in enumerate(lines) if line.strip().startswith("-e")) + 1
    )
