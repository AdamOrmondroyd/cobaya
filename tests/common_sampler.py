...
def body_of_sampler_test(
    ...
    if mpi.is_main_process():
        if sampler_name == "mcmc":
            ...
            ignore_rows = 0
        results = MCSamplesFromCobaya(
            updated_info, products["sample"], ignore_rows=ignore_rows, name_tag="sample"
        )
        clusters = None
        ...
        # 2nd test: clusters
            ...
            if "clusters" in products:
                ...
                assert (
                    0
                ), "Could not check sample convergence: multimodal but no clusters"
        # 3rd test: Evidence
        if "logZ" in products:
...
def body_of_test_speeds(
    ...
    if sampler_name == "polychord":
        ...
        assert (
            test_func(n_evals, dim0, speed0, dim1, 2 * speed1) <= tolerance
        ), "%g > %g" % (test_func(n_evals, dim0, speed0, dim1, speed1), tolerance)
    elif (
        sampler_name == "mcmc"
