...
class BBN(Theory):
    ...
    def calculate(self, state, want_derived=True, **params_values_dict):
        yhe = self.bbn.Y_He(
            self.provider.get_param("ombh2"), self.provider.get_param("nnu") - 3.046
        )
...
class BBN2(Theory):
    ...
    def calculate(self, state, want_derived=True, **params_values_dict):
        if want_derived:
            state["derived"] = {
                "YHe": self.bbn.Y_He(
                    params_values_dict["ombh2"], params_values_dict["nnu"] - 3.046
                )
            }
...
debug = True
info: InputDict = {
    "likelihood": {"cmb": cmb_likelihood_info},
    "theory": {
        "camb": {
...
info_error2: InputDict = {
    "likelihood": {
        "cmb": cmb_likelihood_info,
        "BBN": {"external": BBN_with_theory_errors, "provides": "YHe"},
    },
    "theory": {"camb": {"requires": ["YHe", "ombh2"]}},
...
def test_bbn_likelihood(packages_path, skip_not_installed):
    ...
    assert np.allclose(
        model.loglikes({"YHe": 0.246})[0], [0.246, -0.84340], rtol=1e-4
    ...
    assert np.allclose(
        model.loglikes({"BBN_delta": 1.0})[0], [0.24594834, -0.5], rtol=1e-4
    )
...
class ExamplePrimordialPk(Theory):
    ...
    def calculate(self, state, want_derived=True, **params_values_dict):
        ...
        pk = (self.ks / pivot_scalar) ** (
            params_values_dict["testns"] - 1
        ) * params_values_dict["testAs"]
...
class Pklike(Likelihood):
    def logp(self, **params_values):
        ...
        print(results.Params.scalar_power(1.1), testAs * (1.1 / 0.05) ** (testns - 1))
        assert np.allclose(
            results.Params.scalar_power(1.1),
...
info_pk: InputDict = {
    ...
    "theory": {"camb": {"external_primordial_pk": True}, "my_pk": ExamplePrimordialPk},
    "params": {
        "ombh2": 0.022274,
        "omch2": 0.11913,
        "cosmomc_theta": 0.01040867,
