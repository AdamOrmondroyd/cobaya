...
def test_H0_riess2018a(packages_path, skip_not_installed):
    body_of_test(
        packages_path, like_name="H0.riess2018a", skip_not_installed=skip_not_installed
    )


def test_H0_riess201903(packages_path, skip_not_installed):
    body_of_test(
        packages_path, like_name="H0.riess201903", skip_not_installed=skip_not_installed
    )


def test_H0_docs(packages_path, skip_not_installed):
    like_info = yaml_load_file(
        os.path.join(
    ...
    like_info["likelihood"][like_name]["external"] = like_info["likelihood"][like_name][
        "external"
    ].replace("sigma_H0", str(fiducial_H0_std))
    body_of_test(
        packages_path,
        like_info=like_info["likelihood"],
...
def body_of_test(
    packages_path, like_name=None, like_info=None, skip_not_installed=False, Mb=False
):
    info = {
    ...
    computed_chi2 = products["sample"]["chi2__" + list(info["likelihood"])[0]].values[0]
