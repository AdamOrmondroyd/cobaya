...
half_ring_str = (
    "lambda x, y: stats.norm.logpdf(np.sqrt(x**2 + y**2), loc=0.5, scale=0.1)"
)
...
info_derived = {
    "half_ring": {"external": half_ring_func_derived, "output_params": ["r", "theta"]}
}
...
def body_of_test(info_logpdf, kind, tmpdir, derived=False, manual=False):
    ...
    if derived:
        info["params"].update(
            {"r": {"min": 0, "max": 1}, "theta": {"min": -0.5, "max": 0.5}}
        )
    ...
    logprior_base = -np.log(
        (info["params"]["x"]["prior"]["max"] - info["params"]["x"]["prior"]["min"])
        * (info["params"]["y"]["prior"]["max"] - info["params"]["y"]["prior"]["min"])
    )
    logps = {
        name: logpdf(
            **{
                arg: products["sample"][arg].to_numpy(dtype=np.float64)
