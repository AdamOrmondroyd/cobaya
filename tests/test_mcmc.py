...
def test_mcmc_sync():
    ...
    with pytest.raises((LoggedError, mpi.OtherProcessError)), NoLogging(
        logging.CRITICAL
    ):
...
def test_mcmc_oversampling_manual():
    info_mcmc = {
        "mcmc": {"burn_in": 0, "learn_proposal": False, "oversample_power": 0.4}
    }
...
def _make_gaussian_like(nparam) -> Type[Likelihood]:
    class LikeTest(Likelihood):
        ...
        def calculate(self, state, want_derived=True, **params_values_dict):
            state["logp"] = -np.sum(
                np.array(list(params_values_dict.values())) ** 2 / 2
            )
