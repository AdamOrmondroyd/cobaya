...
info: InputDict = {
    "params": {
        ...
        "joint": {
            "derived": "lambda sigma8, omegam, As100: sigma8 ** 2 * omegam*As100"
        },
    },
    "likelihood": {
        "test_likelihood": {
...
def test_cosmo_run_resume_post(tmpdir, packages_path=None):
    ...
    post_from_text = dict(
        info_post, skip=0, suffix="getdist2"
    )  # getdist already skipped
    output_info, products = run(
    ...
    assert (
        samp_thin.numrows == samp_revert.numrows + products["stats"]["points_removed"]
    )
